...
export const getRealTimePrice = async (ticker: string): Promise<StockData | null> => {
    // console.log("Server Action: Fetching data for", ticker);

    if (!API_KEY) {
        console.error('Server Action: EODHD API Key is missing');
        return null;
    }
...
export const getBulkRealTimePrices = async (tickers: string[]): Promise<StockData[]> => {
    ...
    // EODHD supports bulk via 's' parameter: /real-time/AAPL.US?s=V.US,MSFT.US,...
    // The first ticker is in the path, others in 's' query param.
    ...
    try {
        ...
        const data = await response.json();
        // API returns array if multiple, object if single. Ensure array.
